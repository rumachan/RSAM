...
if (len(sys.argv) < 7) | (len(sys.argv) > 9):
  sys.exit("syntax rsam_plot_day.py site(DRZ.10-EHZ.CH) rsam_dir year1(yyyy) year2(yyyy) plot_dir filter(lp,hp,bp,none) [f1 f2]")
else:
  site = sys.argv[1]
  rsam_dir = sys.argv[2]
...
for year in years:
  ...
  if filtype == 'none':
    ...
    rsamfile = os.path.join(rsam_dir, site_dir, y + '.' + site + '.' + filtype + '_' + strf + '.rsam')
  elif filtype == 'bp':
    strf1 = '%.2f' %f1    #string version with 2 decimal places
    strf2 = '%.2f' %f2    #string version with 2 decimal places
    rsamfile = os.path.join(rsam_dir, site_dir, y + '.' + site + '.' + filtype + '_' + strf1 + '-' + strf2 + '.rsam')
...
if filtype == 'none':
  ...
  title = 'RSAM: ' + site + ', date: ' + y1 + '-' + y2 + ' UT, filter: ' + filtype + ', last value: ' + lastval + ' at ' + lasttim
elif (filtype == 'lp') | (filtype == 'hp'):
  #title = 'RSAM: ' + site + ', date: ' + y1 + '-' + y2 + ' UT, filter: ' + filtype + ' ' + strf + ' Hz' + ', plotted at: ' + now.strftime("%Y-%m-%d %H:%M")
  title = 'RSAM: ' + site + ', date: ' + y1 + '-' + y2 + ' UT, filter: ' + filtype + ' ' + strf + ' Hz' + ', last value: ' + lastval + ' at ' + lasttim
elif filtype == 'bp':
  #title = 'RSAM: ' + site + ', date: ' + y1 + '-' + y2 + ' UT, filter: ' + filtype + ' ' + strf1 + ' - ' + strf2 + ' Hz' + ', plotted at: ' + now.strftime("%Y-%m-%d %H:%M")
  title = 'RSAM: ' + site + ', date: ' + y1 + '-' + y2 + ' UT, filter: ' + filtype + ' ' + strf1 + ' - ' + strf2 + ' Hz' + ', last value: ' + lastval + ' at ' + lasttim 
