...
if (len(sys.argv) < 7) | (len(sys.argv) > 9):
  sys.exit("syntax rsam_plot.py site(DRZ.10-EHZ.CH) rsam_dir date1(yyyymmdd) date2(yyyymmdd) plot_dir filter(lp,hp,bp,none) [f1 f2]")
else:
  site = sys.argv[1]
  rsam_dir = sys.argv[2]
  date1 = sys.argv[3]
...
for date in dates:
  ...
  if filtype == 'none':
    ...
    rsamfile = os.path.join(rsam_dir, site_dir, yd + '.' + site + '.' + filtype + '_' + strf + '.rsam')
  elif filtype == 'bp':
    strf1 = '%.2f' %f1    #string version with 2 decimal places
    strf2 = '%.2f' %f2    #string version with 2 decimal places
    rsamfile = os.path.join(rsam_dir, site_dir, yd + '.' + site + '.' + filtype + '_' + strf1 + '-' + strf2 + '.rsam')
...
if filtype == 'none':
  title = 'RSAM: ' + site + ', date: ' + date1 + '-' + date2 + ' UT, filter: ' + filtype + ', plotted at: ' + now.strftime("%Y-%m-%d %H:%M")
elif (filtype == 'lp') | (filtype == 'hp'):
  title = 'RSAM: ' + site + ', date: ' + date1 + '-' + date2 + ' UT, filter: ' + filtype + ' ' + strf + ' Hz' + ', plotted at: ' + now.strftime("%Y-%m-%d %H:%M")
elif filtype == 'bp':
  title = 'RSAM: ' + site + ', date: ' + date1 + '-' + date2 + ' UT, filter: ' + filtype + ' ' + strf1 + ' - ' + strf2 + ' Hz' + ', plotted at: ' + now.strftime("%Y-%m-%d %H:%M")
