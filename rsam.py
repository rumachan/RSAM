...
if (len(sys.argv) < 7) | (len(sys.argv) > 9):
  sys.exit("syntax rsam.py site(DRZ.10-EHZ.CH) data_dir date(yyyymmdd) out_dir response filter(lp,hp,bp,none) [f1 f2]")
else:
  site = sys.argv[1]
  base_dir = sys.argv[2]
  date = sys.argv[3]
...
data_dir = os.path.join(base_dir,dd.strftime("%Y/%Y.%j"), site_dir)	#/geonet/seismic/mseed/2011/2011.091/DRZ.CH
name = dd.strftime("%Y.%j") + '.' + site + '.D'	#2011.091.DRZ.10-EHZ.CH.D
datafile = os.path.join(data_dir, name)	#/geonet/seismic/mseed/2011/2011.091/DRZ.CH/2011.091.DRZ.10-EHZ.CH.D
...
if filtype == 'none':
  rsam_name = dd.strftime("%Y.%j") + '.' + site + '.rsam'	#2011.091.DRZ.10-EHZ.CH.rsam
elif (filtype == 'lp') | (filtype == 'hp'):
  strf = '%.2f' %f	#string version with 2 decimal places
  rsam_name = dd.strftime("%Y.%j") + '.' + site + '.' + filtype + '_' + strf + '.rsam'	#2011.091.DRZ.10-EHZ.CH.freq_2.0-10.0.rsam
elif filtype == 'bp':
  strf1 = '%.2f' %f1	#string version with 2 decimal places
  strf2 = '%.2f' %f2	#string version with 2 decimal places
  rsam_name = dd.strftime("%Y.%j") + '.' + site + '.' + filtype + '_' + strf1 + '-' + strf2 + '.rsam'	#2011.091.DRZ.10-EHZ.CH.freq_2.0-10.0.rsam
rsampath = os.path.join(out_dir, site_dir)	#out_dir/DRZ.CH
rsamfile = os.path.join(out_dir, site_dir, rsam_name)	#out_dir/DRZ.CH/2011.091.DRZ.10-EHZ.CH.rsam
#print rsamfile
