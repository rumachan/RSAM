...
if (len(sys.argv) < 5) | (len(sys.argv) > 7):
  sys.exit("syntax rsam_data.py site(DRZ.10-EHZ.CH) rsam_dir date(yyyymmdd) filter(lp,hp,bp,none) [f1 f2]")
else:
  site = sys.argv[1]
  rsam_dir = sys.argv[2]
  date = sys.argv[3]
...
rsamfile = os.path.join(rsam_dir, site_dir, rsam_name)   #rsam_dir/DRZ.CH/2011.091.DRZ.10-EHZ.CH.rsam
#print rsamfile
